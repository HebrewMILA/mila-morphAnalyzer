...
public class AdjectiveGen extends ItemGen {
	...
	private void analyseExceptionList(List exceptionList) throws Exception {
		for (int i = 0; i < exceptionList.size(); i++) {
			...
			// lexicon item
			exceptionInflectConstructS = nounExceptionType.isInflectConstructS();
			exceptionInflectConstructP = nounExceptionType.isInflectConstructP();
			...
			if (exceptionBase.startsWith("w") && !exceptionBase.startsWith("ww")) {
	...
	private void inflectAddException(String action, String exceptionBase) throws Exception {

		String exceptionFeminineBase = "";
		String exceptionPluralBase = "";

		if (!construct.equals("true") && exceptionPluralBase.equals("")) {
		...
		if (exceptionInflectConstructS && construct.equals("false")
				&& (exceptionBase.charAt(exceptionBase.length() - 1) != 'i') && lexiconNumber.equals("singular")) {
			setAttributes(lexiconGender, lexiconNumber, "true", "tf", exceptionBase);
			generateConstruct();
		}

		if (lexiconGender.equals("masculine") && !feminine.equals("unspecified")) {
			setAttributes("feminine", lexiconNumber, "false", "tf", exceptionBase);
			generateFeminine();
			exceptionFeminineBase = inflectedItem;
			setAttributes("feminine", lexiconNumber, "false", "tt", exceptionFeminineBase);
			addH();
		}

		if (!plural.equals("unspecified")) {
			setAttributes(lexiconGender, "plural", "false", "tf", exceptionBase);
			generatePlural(plural);
			exceptionPluralBase = inflectedItem;
			setAttributes(lexiconGender, "plural", "false", "tt", exceptionPluralBase);
			addH();
		}
		...
		if (!exceptionFeminineBase.equals("")) {
			...
			if (exceptionInflectConstructS) {
				setAttributes("feminine", "singular", "true", "tf", exceptionFeminineBase);
				generateConstruct();
			}

			setAttributes("feminine", "plural", "false", "tf", exceptionFeminineBase);
			generatePlural("wt");
			String exceptionPluralFeminineBase = inflectedItem;

			setAttributes("feminine", "plural", "false", "tt", exceptionPluralFeminineBase);
			addH();

			if (exceptionInflectConstructP) {
				setAttributes("feminine", "plural", "true", "tf", exceptionPluralFeminineBase);
				generateConstruct();
			}
		...
		if (!exceptionPluralBase.equals("")) {
			if (number.equals("plural") && exceptionInflectConstructP) {
				setAttributes(lexiconGender, "plural", "true", "tf", exceptionPluralBase);
				generateConstruct();
			}
		}
	}
	...
	/**
	 * This method is called before populating the database with a generated item
	 * <br>
	 ...
	 * generated item <br>
	 * If there is a remove item - the generated item will not be inserted to the
	 * database <br>
	 * The method compares the remove/replace exception item gender and number with
	 * <br>
	 ...
	 * @throws Exception
	 */
	private boolean replaceRemoveException(String gender, String number, String construct, List exceptionList,
		...
		if (exceptionList != null) {
			for (int i = 0; i < exceptionList.size(); i++) {
				 ...
				 * System.out.println("construct  =" + construct); System.out.println("PGN  =" +
				 * PGN);
				 */
				if (exceptionGender.equals(gender) && exceptionNumber.equals(number)
						&& (exceptionConstruct.equals(construct)
								|| (exceptionConstruct.equals("false") && construct.equals("unspecified"))
								|| (exceptionConstruct.equals("unspecified") && construct.equals("false")))) {
	...
	private void handleDoublingVavForAdjectivesStartsWithVav() throws Exception {
		if (transliterated.startsWith("w") && !transliterated.startsWith("ww")) {
			// System.out.println("|||||||||||||||||||||||||||||||||||||");
			// System.out.println("||||||||||double VAV for nouns starts with single
			// VAV||||||||||||");
	...
	private void generateConstruct() throws Exception {
		...
		if (!replaceRemoveException(lexiconGender, lexiconNumber, "true", replaceExceptionList, "replace")) {
	...
	private void generatePlural(String pluralSuffix) throws Exception {
		...
		if (!replaceRemoveException(gender, "plural", "unspecified", replaceExceptionList, "replace")) {
	...
	private void generateFeminine() throws Exception {
		...
		if (!replaceRemoveException("feminine", number, "unspecified", replaceExceptionList, "replace")) {
	...
	private void inflectLexiconItem() throws Exception {
		...
		if ((lexiconNumber.equals("singular") && inflectConstructS)
			...
			setAttributes(lexiconGender, lexiconNumber, "true", "unspecified", transliterated);
			generateConstruct();
		}
		...
		if (!plural.equals("unspecified") && !lexiconNumber.equals("plural")) {
			...
			generatePlural(plural);
			setAttributes(lexiconGender, "plural", "false", "tt", inflectedItem);
			addH();
		}
		...
		if (!feminineBase.equals("")) {
			...
			if (inflectConstructP) {
				setAttributes("feminine", "plural", "true", "unspecified", pluralFeminineBase);
				generateConstruct();
			}
		}
