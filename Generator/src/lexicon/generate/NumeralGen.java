...
public class NumeralGen extends ItemGen {
	...
	protected boolean replaceExceptionExist() throws Exception {
		...
		for (int i = 0; i < replaceExceptionList.size(); i++) {
			...
			numeralExceptionType.open(((Integer) replaceExceptionList.get(i)).intValue());
			...
			if (exceptionGender.equals(gender) && exceptionConstruct.equals(construct) && exceptionNumber.equals(number)) {
	...
	private void analyseExceptionList(List exceptionList) throws Exception {
		for (int i = 0; i < exceptionList.size(); i++) {
			NumeralExceptionType numeralExceptionType = new NumeralExceptionType();
			numeralExceptionType.open(((Integer) exceptionList.get(i)).intValue());
			inflectedItem = numeralExceptionType.getTransliterated();
			surface = numeralExceptionType.getUndotted();
			register = numeralExceptionType.getRegister();
			spelling = numeralExceptionType.getSpelling();
			...
			if (construct.equals("false") && (PGN.equals("unspecified") || PGN.equals(""))) {
				setAttributes(gender, "plural", "false", "tt", inflectedItem, "unspecified", "unspecified");
	...
	protected void generatePlural() throws Exception {
		...
		if (type.equals("numeral ordinal")) {
			...
			if (gender.equals("masculine")) {
				...
				inflectedItem = inflectedItem.substring(0, inflectedItem.length() - 1) + "wt";
				surface = Translate.Eng2Heb(inflectedItem);
			}
			// System.out.println();
			// System.out.println("inflectedItem =" + inflectedItem);
	...
	public void generateInflects() throws Exception {
		...
		setAttributes(gender, number, construct, "tt", inflectedItem, "unspecified", "unspecified");

		addH();

		setAttributes(gender, number, "true", "unspecified", transliterated, "unspecified", "unspecified");

		generateConstruct();

		setAttributes(gender, number, "false", "tf", transliterated, "unspecified", "possessive");

		generatePossessive();

		if (!gender.equals("feminine") && (type.equals("numeral ordinal"))) {

			setAttributes("feminine", "singular", "false", "tf", transliterated, "unspecified", "unspecified");
		...
		if (type.equals("numeral ordinal") || type.equals("numeral fractional"))
			if (!number.equals("plural")) {
				...
				setAttributes(item.getNumeral().getGender(), "plural", "false", "tf", transliterated, "unspecified",
						"unspecified");
		...
		if (!feminineBase.equals("")) {
			...
			setAttributes("feminine", number, "false", "tt", feminineBase, "unspecified", "unspecified");
			...
			setAttributes("feminine", "plural", "false", "tt", inflectedItem, "unspecified", "unspecified");

			addH();

			setAttributes("feminine", "plural", "true", "unspecified", feminineBase, "unspecified", "unspecified");

			generateConstruct();

			setAttributes("feminine", "plural", "false", "tf", feminineBase, "unspecified", "possessive");
		...
		if (!pluralBase.equals("")) {
			...
			setAttributes(gender, "plural", "false", "tt", pluralBase, "unspecified", "unspecified");

			addH();

			setAttributes(gender, "plural", "true", "unspecified", pluralBase, "unspecified", "unspecified");

			generateConstruct();

			setAttributes(gender, "plural", "false", "tf", inflectedItem, "unspecified", "possessive");
