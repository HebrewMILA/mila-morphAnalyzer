 ...
 *         databse) <br>
 * 
 *         Noun generation is very simmilar to adjective generation with the
...
public class NounGen extends ItemGen {
	...
	private void analyseExceptionList() throws Exception {
		...
		for (int i = 0; i < size; i++) {
			...
			if (nounExceptionType.getTransliterated().startsWith("w")
					&& !nounExceptionType.getTransliterated().startsWith("ww")) {
	...
	private void inflectAddException(String exceptionBase) throws Exception {
		...
		if (inflectExceptionConstructS && construct.equals("false") && PGN.equals("unspecified")) {
			setAttributes(lexiconGender, lexiconNumber, "true", "unspecified", exceptionBase, "unspecified",
					"unspecified");
			generateConstruct();
		}
		...
		if (!exceptionFeminineBase.equals("")) {
			...
			if (inflectExceptionConstructS) {
				setAttributes("feminine", "singular", "true", "unspecified", exceptionFeminineBase, "unspecified",
						"unspecified");
				generateConstruct();
			}
			...
			if (inflectExceptionConstructP) {
				setAttributes("feminine", "plural", "true", "unspecified", exceptionPluralFeminineBase, "unspecified",
						"unspecified");
				generateConstruct();
			}

			if (inflectExceptionPossessiveP) {
				setAttributes("feminine", "plural", "false", "tf", exceptionPluralFeminineBase, "unspecified",
						"possessive");
				generatePossessive();
			}
		}
		...
		if (!exceptionPluralBase.equals("")) {
			...
			if (inflectExceptionConstructP) {
				setAttributes(lexiconGender, "plural", "true", "unspecified", exceptionPluralBase, "unspecified",
						"unspecified");
				generateConstruct();
			}
			if (inflectExceptionPossessiveP) {
	 ...
	 * database <br>
	 * The method compares the remove/replace exception item PGN,gender and number
	 * with <br>
	...
	private boolean replaceRemoveException(String PGN, String gender, String number, String construct,
		...
		if (exceptionList != null) {
			for (int i = 0; i < exceptionList.size(); i++) {
				 ...
				 * System.out.println("!!!!!!!!!!!!!!!!!!!!!!!!exceptionConstruct  ="+
				 * exceptionConstruct); System.out.println("construct  =" + construct);
	...
	private boolean generateConstruct() throws Exception {
		...
		boolean matchRemove = replaceRemoveException("unspecified", gender, number, "true", removeExceptionList,
				"remove");
		boolean matchReplace = replaceRemoveException("unspecified", gender, number, "true", replaceExceptionList,
				"replace");
		...
		if (!matchReplace && !matchRemove) {
			...
			if (inflectConstructP
					&& (number.equals("plural") || number.equals("dual") || number.equals("dual and plural"))) {
				...
				// System.out.println("(F) generateConstruct() : inflectedItem = " +
				// inflectedItem);
				populateDatabase();
				...
				// System.out.println("(F) generateConstruct() : surface = " + surface);
				populateDatabase();
			} else if (gender.equals("feminine")) { // טיפול בקריה יש צורך ליצור קריית
				if (inflectedItem.endsWith("ih")) {
					// System.out.println("(F) generateConstruct() : GOING TO CREATE DOUBLE i");
					// System.out.println("(F) generateConstruct() : surface = " + surface);
				}
		...
		if (inflectPossessiveP || inflectPossessiveS) {
			...
			if ((number.equals("singular") && inflectPossessiveS) || ((number.equals("plural") && inflectPossessiveP))
				...
				while (stPGN.hasMoreTokens()) {
					...
					if (!replaceRemoveException(PGN, gender, number, "false", removeExceptionList, "remove")
							&& !replaceRemoveException(PGN, gender, number, "false", replaceExceptionList, "replace")) {
	...
	private void setAttributes(String gender, String number, String construct, String definitnessVal,
			String inflectedItem, String PGN, String suffixFunction) {
