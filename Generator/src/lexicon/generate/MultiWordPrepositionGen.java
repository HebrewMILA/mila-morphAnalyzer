...
public class MultiWordPrepositionGen extends ItemGen {
	...
	protected void inflectPronomial(String transliteratedLexiconItem, String surfaceLexiconItem, String inflected,
		...
		while (stSuff.hasMoreTokens()) {
			...
			String surface = Translate.Eng2Heb(transliterated1) + " " + Translate.Eng2Heb(inflected + suffix);
			...
			// }
			// else
			popualteMWE.popualteMWETables(transliteratedLexiconItem, surfaceLexiconItem, '1', "preposition", "unspecified",
					id, "unspecified", PGN, spelling, register, inflectedItem, surface);
	...
	public void generateInflects() throws Exception {
		...
		try {
			...
			popualteMWE.popualteMWETables(transliterated, surface, '1', mwPos, dottedLexiconItem, id, type, spelling,
					register, false);
			inflectPronomial(transliterated, surface, transliterated2, transliterated1);
		} catch (NoSuchElementException e) {
			System.err
					.println("Error: Couldn't tokenize transliterated string! <" + transliterated + "> (" + this.id + ")");
			throw e;
		}
