...
public class MultiWordFrozenGen extends ItemGen {
	 ...
	 * transliterated;
	 * 
	 * //definitnessVal = "true"; popualteMWE.popualteMWETables( mwTransliterated,
	 * mwUndotted, '1' , mwPos , dottedLexiconItem, id, type, spelling, register,
	...
	public void generateInflects() throws Exception {
		...
		popualteMWE.popualteMWETables(transliterated, undottedLexiconItem, '1', mwPos, dottedLexiconItem, id, type,
		...
		if (acceptDefiniteness) {
			...
			popualteMWE.popualteMWETables(mwTransliterated, mwUndotted, '1', mwPos, dottedLexiconItem, id, type, spelling,
		...
		String sql = "SELECT count(*) FROM lexiconP.multiWordFrozen_exception_type where id=" + item.id; // i want to find
																																			// out how many
																																			// exception
																																			// this item
		...
		if (rs != null) {
			if (rs.next()) {
				...
				if (exception_count != 0) // if we have one or more exceptions
				{
					...
					String sql2 = "SELECT aid FROM lexiconP.multiWordFrozen_exception_type where id=" + item.id; // i want to
																																				// get the
																																				// aid for
																																				// exceptions
					// this item have
					...
					if (rs != null) {
						...
						for (int aid : aids) {
							...
							definitnessVal = "false";
							popualteMWE.popualteMWETablesOneWord(_transliterated, _undotted, '1', mwPos, _dotted, id, type,
							...
							if (acceptDefiniteness) {
								...
								popualteMWE.popualteMWETablesOneWord(mwTransliterated, mwUndotted, '1', mwPos, _dotted, id,
										type, _spelling, _register, prefix, definitnessVal);
							}
