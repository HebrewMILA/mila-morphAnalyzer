...
public class rules1 extends ConnectedGenerator {
	 ...
	 * Possessive rules have a separete handling in defaultPossessiveRulesHandling()<br>
	 */
	private void getDefaultRule() {
		String inflectedPattern = "";
		String sql = "select * from rules1 where  action = '" + action
				+ "' and inputPattern = 'default'";
		rs = getData(sql);
		int indexToken;
		try {
			while (rs.next()) {
				...
				if (tInputCondition.equals(inputCondition)) {
						...
						inflectedTransliterated = inflectedTransliterated
								+ tInflectedPattern;
	...
	private void defaultPossessiveRulesHandling() {
		...
		while (stDefSuff.hasMoreTokens()) {
			...
			if (defSuffix.equals("-"))
				defSuffix="";
			inflectedTransliterated = inflectedTransliterated + base
					+ defSuffix + ",";
...
//			else if (index == -1) {
				if (subLast.equals(tInputPattern)
						&& inputCondition.equals(tInputCondition)) 
				{
...
//			}
	...
	{	
		...
		//System.out.println("(F) buildMatch inflectedTransliterated =" + inflectedTransliterated);
		/*
		try 
		{
			System.out.println("(F) buildMatch rule= " + rs.getString("comment") +" id= " + rs.getString("id") );
		} 
		catch (SQLException e) 
		{
			e.printStackTrace();
	...
	public void getRules() 
	{
		...
		String sql = "select * from rules1 where  action = '" + action + "' and inputPattern != 'default' order by inputSuffixLen DESC";
