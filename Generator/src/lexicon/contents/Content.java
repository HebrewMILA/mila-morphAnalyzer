...
/**
 * The base class for all the content items in the system, including ItemType, MetadataType, User etc.
 * The class enables the following functionality:<p>
 * <ui>
 * <li>Basic content management functionality such as <code>open, update and
 * delete</code>.
 * <li>Get / Set operations for the content fields, which are cached in the object
 * after the <code>open()</code> operation.
 * <li>Database connection functionality, such as <code>getData(sql)</code> and
 * <code>execute</code>. Furthermore, the class handles such DB conenction functionality
 * as connection pooling management, direct connection management, and connection
 * configuration using <code>.ini</code> file.
 * </ui>
 * <p>
 ...
 * object. The owner might be the user who created the object.
 * </ui>
...
{	
	...
	/**
	*Represent the ID of the content. The id name in the DB is different for different types
	*of content, and it may be iid for item or eid for event
	*/
	public int id = 0;;

	/**
	*Holds the content data from the DB. Each data field is accesible using set() and
	*getString() / getInt() methods, using the same name as in the DB record.
	*/
	...
	*/
	...
	/**
	*The name of the id of the current content, such as iid, eid etc.
	*The field is set by the concrete class constructor.
	*/
	...
	*/	
	...
	/**
	* Delete from the DB all the records that are connected with the current object.
	* Any overide should take care of deleting any sub tables such as exception_type, english, etc.
	* @return	The accumalated number of rows that were affected from the action.
	* If 0, then nothing happened
	*/
	public int remove() 
	{
		int feedback = 0;
	...
	* @param		table - the name of the requestred table
	* @param		idName - the name of the coulumn title (id, sid...)
	...
	*The method updates all the fields of the Content.
	*@see   #info
	...
	*/
	...
	/**
	*Return the table name for the current content object. For example, returns "items" for
	*for the Item class.
	...
	*/
	...
	/**
	*Return the id name for the current content object. For example, returns "iid" for
	*for the Item class.
	...
	*/
	...
	/**
	*Populates the current object variables with the variables from the DB record. Translate the
	*ResultSet record into a the HashMap <code>info</code> state variable. The HashMap saves
	*the content object data fields, are gives access to them using the set, and get* (getInt,
	*for example) methods.<br>
	*The method populates the fields according to the field types. Up to now, the following fields
	*are supported: String, blob (treated the same way as String), int, float, date. In order to support more field types, an update
	*to the method is needed.
	*@param		rs	The record set of the current object.
	*@see		#open
	*@see           #set
	...
	*/
	protected void openRS(ResultSet rs) {
		...
		try {
			...
			for (int i = 1; i <= meta.getColumnCount(); i++) {
				...
				else {System.out.println("ERROR in cTYPE (Content.OpenRS) "+cType);}
				//System.out.println(meta.getColumnName(i) + " = " + rs.getObject(i) + " ("+ cType +")");
			}
		} catch (SQLException E) {
			E.printStackTrace();
		}
	...
	/**
	 * The method opens a DB record using a passed ID, and populates the
	 * current object with the data from that record. The method uses <code>openRS</code>
	 * to populate the <code>info</code> and <code>id</code> variables with the data.
	 * Note that this method is highly "expensive" comparing to the <code>openRS</code> method.
	 * @param   requestedID   The id of the record to be opened.
	 * @see     #openRS
	 */
	 ...
	 * @param   key     The key to retrieved the value by.
	...
	/**
	 * Retreived a String value from the <code>info</code> HashMap object.
	 * If the method does not find a value by the passed key, then it returns
	 * an empty string ("").
	 * @param   key     The key to retrieved the value by.
	...
	/**
	 * Retreived an int value from the <code>info</code> HashMap object.
	 * If the method does not find a value by the passed key, then it returns
	 * 0.
	 * @param   key     The key to retrieved the value by.
	 * @return  An int value.
	...
	/**
	 * Retreived a float value from the <code>info</code> HashMap object.
	 * If the method does not find a value by the passed key, then it returns
	 * 0.
	 * @param   key     The key to retrieved the value by.
	...
	/**
	 * Retreived a double value from the <code>info</code> HashMap object.
	 * If the method does not find a value by the passed key, then it returns
	 * 0.
	 * @param   key     The key to retrieved the value by.
	...
	/**
	 * Retreived a date object from the <code>info</code> HashMap object.
	 * If the method does not find a value by the passed key, then it returns
	 * null.
	 * @param   key     The key to retrieved the value by.
	...
	/**
	 * Sets a String value in the <code>info</code> HashMap object.
	 * If the passed <code>value</code> is null, then the empty string is
	 * inserted. If the insert operation on the HashMap throws a <code>
	 * NullPointerException</code> exception, then an error message is printed
	 * to System.out.
	 * @param   key     The key to to set the value by.
	...
	/**
	 * Sets an int value in the <code>info</code> HashMap object.
	 * If the insert operation on the HashMap throws a <code>
	 * NullPointerException</code> exception, then an error message is printed
	 * to System.out.
	 * @param   key     The key to to set the value by.
	 * @param   value   The value to be set.
	...
	/**
	 * Sets a float value in the <code>info</code> HashMap object.
	 * If the insert operation on the HashMap throws a <code>
	 * NullPointerException</code> exception, then an error message is printed
	 * to System.out.
	 * @param   key     The key to to set the value by.
	...
	/**
	 * Sets a double value in the <code>info</code> HashMap object.
	 * If the insert operation on the HashMap throws a <code>
	 * NullPointerException</code> exception, then an error message is printed
	 * to System.out.
	 * @param   key     The key to to set the value by.
	...
	/**
	 * Sets a Date value in the <code>info</code> HashMap object.
	 * If the insert operation on the HashMap throws a <code>
	 * NullPointerException</code> exception, then an error message is printed
	 * to System.out.
	 * @param   value   The value to be set.
	 ...
	 *@return <code>true</code> if the object has the field. <code>false</code> otherwise.
	 */
	 ...
	 * otherwise.
	 * @param   key     The name of the field (should correspond with the DB
	 * name).
	 */
	...
	/**
	 * Gets a list on EmptyContents with the info populated according of the table's fields.
	 * @param tableName - the table from which we get the infrmation 
	 * @param idName - the name of the field using fot the "WHERE" clause
	 * @param id the id to compare the idName to.
	 * @return
	 */
	...
	/**
	 * removes a list of contents sharing the same idName1=id, and their own idNAme=idNaem2
	 * @param tableName
	 * @param idName1 - the common field of the list
	 * @param idName2 - the idname of the the row to be deleted
	 * @param id
	 * @return
	 */
