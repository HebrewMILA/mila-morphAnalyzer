...
<%@taglib prefix="hlp" uri="http://mila.technion.ac.il/helpers"%>
...
<sql:query var="bad_words"
	sql="SELECT multiWord.id AS id, item.undotted AS undotted, multiWord.pos AS pos
		 ...
		 multiWord.pos NOT IN ('noun', 'propername', 'adverb', 'interjection', 'unspecified', 'preposition', 'adjective', 'verb');" />
<sql:query var="bad_pos_types" 
	sql="SELECT multiWord.pos AS pos, COUNT(multiWord.pos) AS count
		 FROM multiWord, item
		 WHERE multiWord.id = item.id AND
		 multiWord.pos not in ('noun', 'propername', 'adverb', 'interjection', 'unspecified', 'preposition', 'adjective', 'verb')
...
<body>
	...
	<table border="0" align="center">
		...
		<c:forEach var="row" varStatus="loop" items="${bad_pos_types.rows}">
			<tr id="row_${loop.index}" class="${loop.index % 2 == 0 ? 'evenRow' : 'oddRow'}">
				<td class="index"><c:out value="${loop.index}" /></td>
				<td class="english"><c:out value="${row.pos}" /></td>
				<td class="english"><c:out value="${row.count}" /></td>
			</tr>
		</c:forEach>
	</table>
</body>
