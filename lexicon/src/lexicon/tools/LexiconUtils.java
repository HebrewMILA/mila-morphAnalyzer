...
public class LexiconUtils {
	protected static final String[] EXCEPTION_TYPES = { "adverb",
			...
			"quantifier", "verb", "propername", "pronoun", "numeral", "copula",
			"modal", "existential" };

	// ------------------------------------------------------------------------------------------------
	public static ResultSet GetItem(String sql) { // EDIT : not used
													// get the item that match
													// the sql query
		ResultSet rs = null;
		// int id = -1 ;
	...
	public static int GetNextItemId(int current_id) {
		int id = -1;
		final EmptyContent content = new EmptyContent();
		final String sql = "SELECT * FROM item WHERE id > " + current_id
	...
	public static String GetNextItemPos(int current_id) {
		return LexiconUtils.GetItemPos(LexiconUtils.GetNextItemId(current_id));
	}
	...
	public static String GetPrevItemPos(int current_id) {
		return LexiconUtils.GetItemPos(LexiconUtils.GetPrevItemId(current_id));
	}
	...
	public static String getTransliteration(String hebUTF) {
		...
		final String[] heb = hebUTF.substring(1).split("%");
		for (int i = 0; i < heb.length; i++) {
			...
			if (heb[i].startsWith("D")) {
				if (heb[i + 1].length() == 2) {
					...
					final String temp = heb[i + 1].substring(0, 2);
	...
	public static void main(String[] args) {
		...
		final List<SearchItem> result = LexiconUtils.searchForItems("", "",
				undotted, "");
		System.out.println(result.size());

		/*
		 * String trans = null; try { trans =
		 * getTransliteration(URLEncoder.encode("טום וג'רי", "UTF-8")); } catch
		 * (UnsupportedEncodingException e) { // TODO Auto-generated catch block
		 * e.printStackTrace(); } System.out.println(trans);
		 */
	 ...
	 *            result as jsp:forward parameters fields. otherwise, it woudl
	 *            create a link.
	 * @return
	 */
	public static String renderAllRequestParameters(ServletRequest req,
			TreeSet<?> omitList, int doHiddens) {
		if (req == null)
			return "";
	...
	// -------------------------------------------------------------------------------------------------
	public static List<SearchItem> searchForItems(String id, String dotted,
			String undotted, String transliterated) {
		// undotted = undotted.replaceAll("'","&#39;");
		try {
