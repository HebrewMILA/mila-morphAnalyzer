...
if(textFromTextArea!=null){
	try{	
		...
		try{
			in = new java.io.ByteArrayInputStream(textFromTextArea.getBytes("UTF-8"));   // get input
			...
			MorphMult2TaggerFormat mm = new MorphMult2TaggerFormat();  // class for converting to HMM format
                        ...
                        String taggerFormat = mm.myWEBMorp2Tagger(postProcessorResult, tempDirectoryPath); // convert to HMM and save to temp file

                        String taggerInputFile =  tempDirectoryPath + taggerFormat;
                        fileName= taggerFormat.substring(1);   
                        taggedFile = "/data/tagger/royTagger/workdir"+ "/tagging-"+ fileName;
				...
				String command =  "/data/tagger/royTagger/MTTest.pl -dir /data/tagger/royTagger/workdir/  -rmtmp  "+ taggerInputFile  + " /data/tagger//taggerLearningOutputFile/corpus.lm  /data/tagger/taggerLearningOutputFile/corpus.lex.prob " + taggedFile;
			...
			postProcessorResult =  "<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" + "<?xml-stylesheet type=\"text/text\"?>" + postProcessorResult.substring(55) ;
            postProcessorResult = postProcessorResult.replaceAll("<corpus","<corpus pId=\"1\"   sId=\"1\"");
            postProcessorResult=postProcessorResult.trim();
            session.setAttribute("ppoutputAnalysis",postProcessorResult);

			//postProcessorResult = "<?xml-stylesheet type=\"text/xsl\" href=\"www.mila.cs.technion.ac.il:8088/MWE/input1.xsl\"?>" + postProcessorResult.substring(55);
			out.clearBuffer();
			out.println(postProcessorResult);//.replaceAll("<","&lt;"));
			...
			{
				e.printStackTrace();
				System.out.println("An error occured while moving from String to inputString");
			}
       }
       catch (Exception e) 
       {
