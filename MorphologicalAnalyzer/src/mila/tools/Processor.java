...
public class Processor {
	...
	public static void main(String args[]) throws ParseException, IOException,
		...
		System.out.println("Done, encountered "
				+ Integer.toString(proc.getErrorCount()) + " errors.");
		System.exit(0);
	}
	...
	private void processDir(File input, File output, File tmpDir)
			throws IOException, JAXBException {
		if (input.isDirectory()) {
			output.mkdirs();
			for (File f : input.listFiles()) {
				processDir(f, new File(output.getCanonicalPath()
						+ File.separator + input.getName()), tmpDir);
			}

		} else { /* input is file */
			output.mkdirs();
			processFile(input, new File(output.getCanonicalFile()
					+ File.separator + input.getName()), tmpDir);
		}

	}

	private void processFile(File input, File output, File tmpDir)
			throws IOException, JAXBException {
		...
		try {
			for (Tool tool : tools) {
				...
				if (tools.get(tools.size() - 1) == tool) {
					...
					temporary = File.createTempFile("processor", "",
			...
			errors.add(new ProcessorError(e, cur_input.getCanonicalPath(),
					ToolType.TAGGER));
			e.printStackTrace();
		}
	}
